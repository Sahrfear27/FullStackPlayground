...
Given a list of words followed by a maximum number of characters in a line, return a collection of strings where each string element represents a line that contains as many words as possible, with the words in each line being concatenated with a single '-' (representing a space, but easier to see for testing). 
...
function wrapLines(words1, maxLen) {
  let currentWord = [];
  let line = [];
  for (let word of words1) {
    let joinwords = [...currentWord, word].join("-");

    if (joinwords.length > maxLen) {
      line.push(currentWord.join("-"));
      currentWord = [word];
    } else {
      currentWord.push(word);
    }
  }
  if (currentWord.length) line.push(currentWord.join("-"));
  return line;
}
...
let test = [];
let j = [...test, "Hello", "World"].join("-");
// console.log(wrapLines(w1, maxLen));
console.log(j);
